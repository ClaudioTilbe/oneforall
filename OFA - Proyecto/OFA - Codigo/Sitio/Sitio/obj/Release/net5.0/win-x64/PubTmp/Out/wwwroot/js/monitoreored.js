...
*Segun el Curso de SignalR en .NET Core ya no se utiliza JQuery y se comienza a utilizar TypeQuery

*Para poder utilizar "debugger;" al depurar tenemos que habilitar la opcion en el boton de depurar (>), deplegar la
pestaña y entrar a "Depuracion de scripts" y habilitarlo


*Problema al actualizar mi pagina javacript y Css. Al volver a depurar no se actualizan los cambios
Solucion: Depurar, abrir con F12 el inspeccionar, pararse sobre el boton para recargar pagina, dar click derecho 
y seleccionar "Vaciar el cache y volver a cargar de manera forzada"
...
//    return $.extend(mensaje, {
//        Id: mensaje.id.toString(),
//        FechaGenerado: mensaje.fechaGenerado,
//        Contenido: mensaje.contenido,
//        DispositivoIP: mensaje.provieneDispositivo.ip
//    });
//}
...
$(function () {
    $sondeoTable = $('#sondeoTable'),
        ...
        //liTemplateVisor = '<li data-ip="{Id}"><span class="id"><strong> ID({Id})</strong>: </span>  <span class="contenido">{Contenido}. </span> <span class="dispositivoIP"><strong> IP </strong> {DispositivoIP} </span> <span class="fechaGenerado text-primary"> <strong>[Fecha {FechaGenerado}]</strong></span> </li>',
...
//Evento previo a Descargar la pagina, pongo Aqui el CloseSondeo para desconectarme y no consumir recursos
//al abandonar mi index

// Agrega el evento 'beforeunload' al objeto 'window'
window.addEventListener('beforeunload', function (event) {
    ...
    //// Cancela el evento 'beforeunload' para evitar que el navegador muestre un mensaje de confirmación al usuario
    //event.preventDefault();
    //// La propiedad 'returnValue' se utiliza para mostrar un mensaje personalizado en algunos navegadores, aunque su soporte es limitado
    //event.returnValue = '';
});
...
// Otra Funcion ------------------------------------------------------------------------------
//Pase la toma de primeros datos al controlador y agregue data-ip en el HTML

//function actualizoDispositivos() {

//    return connection.invoke("getDispositivos").then(function (dispositivos) {

//        $sondeoTableBody.empty();
//        //$sondeoConexionesUl.empty();
//        $.each(dispositivos, function () {
...
//            //Deberia poder poner el encabezado de data-ip en el HTML y desaserme de este metodo en teoria
//            $sondeoTableBody.append(rowTemplate.supplant(dispositivo));
//            //$sondeoConexionesUl.append(liTemplate.supplant(sondeo));
//        });
//    });
//}
